...
void setup(){
  ...
  delay(2500);               //Give the gyro time to start 
  Serial.println(F("Press any key if Drone is Connected with Application"));
...
void loop(){
  ...
  #if F_CPU == 8000000L          //If the clock speed is 8MHz include the next code line when compiling
    clockspeed_ok = 1;            //Set clockspeed_ok to 1
  #endif                          //End of if statement

  if(TWBR == 12 && clockspeed_ok){
    Serial.println(F("I2C clock speed is correctly set to 200kHz."));
  }
  else{
    Serial.println(F("I2C clock speed is not set to 200kHz. (ERROR 8)"));
    error = 1;
  }
  ...
  if(error == 0){
    ...
    for(float i = 900;i > 0;i--){
      delay(10);
      Serial.print(i/100);
      Serial.print(" ");
      receiver_data();
      // code for wifi receiver to be formed
    }
    Serial.println(" ");
    ...
    Serial.print(F("Receiver input 1 = "));
    Serial.println(receiver_input_channel_1);
    Serial.print(F("Receiver input 2 = "));
    Serial.println(receiver_input_channel_2);
    Serial.print(F("Receiver input 3 = "));
    Serial.println(receiver_input_channel_3);
    Serial.print(F("Receiver input 4 = "));
  ...
  if(error == 0){  
    ...
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Move the roll stick to simulate right and back to center"));
    //Check for throttle movement
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Move the pitch stick to simulate down and back to center"));
    //Check for throttle movement
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Move the yaw stick to simulate right and back to center"));
    //Check for throttle movement
  ...
  if(error == 0){
    ...
    if(error == 0){
      ...
      Serial.println(F("Move pitch stick 'nose up' and back to center to continue"));
      check_to_continue();
    ...
    if(error == 0){
      ...
      Serial.println(F("Put the quadcopter back in its original position"));
      Serial.println(F("Move pitch stick 'back' and back to center to continue"));
      check_to_continue();
  ...
  if(error == 0){
    ...
    Serial.println(F("The LED should now be alternatively Blinking"));
    Serial.println(F("The Connection with the application would be automatically Disconnected..."));
    Serial.println(F("You can now calibrate the rotors and upload the DroneTreX Main code."));
    while(1)
  {
    leds_blue_status(0);
    leds_red_status(1);
   ...
   Serial.println(F("Check the user manual for DroneTreX:"));
   Serial.println(F("Mail us at protrex.info@gmail.com for more information about this error. visit: www.protrex.in"));
  }
  while(1);
}
...
void check_receiver_inputs(byte movement){
  ...
  while(timer > millis() && trigger == 0){
     // code for wifi remote receiver needs to be formed
     receiver_data();
    delay(250);
...
void intro(){
  ...
  delay(1500);
  Serial.println(F(""));
  Serial.println(F("ProTreX"));
  delay(500);
  Serial.println(F("DroneTreX Mini Drone"));
  delay(500);
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("DroneTreX Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F("For support and questions: protrex.info@gmail.com"));
  Serial.println(F(""));
...
{
  RemoteXY_Handler();
  receiver_input_channel_1 = map(RemoteXY.Stick2_y,-100,100,1000,2000);
  receiver_input_channel_2 = map(RemoteXY.Stick1_x,-100,100,1000,2000);
  receiver_input_channel_3 = map(RemoteXY.Stick1_y,-100,100,1000,2000);
  receiver_input_channel_4 = map(RemoteXY.Stick2_x,-100,100,1000,2000);
